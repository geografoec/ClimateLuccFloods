...
#    y del modelo IPSL (historical data). Hacer un split de la serie 70:30 (calibracion:validaci?n).
...
plot(obs$date, obs$precip, type="l", col="black",
...
# historical daily precipitation CMIP6-IPSL from 1980-01-01 to 2014-12-31

# load data 
historical <- "./Methods/pr_day_IPSL-CM6A-LR_historical_r1i1p1f1_gr_19800101-20141231_v20180803.nc"
...
# fitting a transfer function, using the series (observed and modeled) in the same period (1980-2010)
qm.fit <- fitQmapQUANT(train$pobs,train$phist,
                       qstep = 0.1,nboot = 1,wet.day = TRUE); qm.fit

# using fitted function for historical test data BC of IPSL model
...
plot(cfcpobs$cfcpobs,dt1, xlim = c(0, 150),type="l", col ="red", lwd=3, 
     main="Cumulative frequency Curve", xlab="", ylab="");par(new=TRUE) # to overlap plots

# CFC of historical (IPSL)
plot(cfcphist$cfcphist,dt1, xlim = c(0, 150),type="l", col ="blue", lwd=1,
...
# CFC observed
plot(cfcpobs$cfcpobs,dt1, xlim = c(0, 200),type="l", lwd=2, 
     main="", xlab="", ylab="");par(new=TRUE) 

#CFC historical-raw
plot(cfcphist$cfcphist,dt1, xlim = c(0, 200),type="l", lwd=2, lty=2, col="blue",
     main="", xlab="", ylab="");par(new=TRUE) 

#CFC historical-BC
plot(cfcph.bc$cfcph.bc,dt1, xlim = c(0, 200),type="l", lwd=2,col="red",
     main="", xlab="", ylab="");par(new=TRUE) 
...
mkt.obs <- mk.test(test$pobs); mkt.obs    #> p<0.05; SI hay tendencia
...
hist.daily <- zoo(df.daily$phist, order.by = df.daily$date2) # historical raw test data (ipsl model) 
...
# daily precipitation CMIP6-IPSL-SSP5-8.5 from 2015-01-01 to 2070-12-31
# note: once the transfer function is ready, we can apply it to the future scenario time series

# load data 
future <- "./Methods/pr_day_IPSL-CM6A-LR_ssp585_r1i1p1f1_gr_20150101-20701231_v20190903.nc"
...
#write.csv(df.ssp,"./BiasCorrection/Routputs/rcode8_daily_ipslssp585_bcm1219splited.csv",row.names = FALSE)
#save.image("./BiasCorrection/RWorkingSpace/rcode8_wspace.RData")
