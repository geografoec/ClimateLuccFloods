...
#    y del modelo NorESM2 (historical data). Hacer un split de la serie 70:30 (calibracion:validaci?n).
...
# historical daily precipitation CMIP6-NorESM2 from 1980-01-01 to 2014-12-31

# load data 
historical <- "./Methods/pr_day_NorESM2-MM_historical_r1i1p1f1_gn_19800101-20141231_v20191108.nc"
...
prec$phist[is.na(prec$phist)] <- 0 # para no tener NA
...
# using fitted function for historical test data BC of NorESM2 model
...
plot(cfcpobs$cfcpobs,dt1, xlim = c(0, 250),type="l", col ="red", lwd=3, 
     main="Cumulative frequency Curve", xlab="", ylab="");par(new=TRUE) # to overlap plots

# CFC of historical (NorESM2)
plot(cfcphist$cfcphist,dt1, xlim = c(0, 250),type="l", col ="blue", lwd=1,
...
hist.daily <- zoo(df.daily$phist, order.by = df.daily$date2) # historical raw test data (NorESM2 model) 
...
# daily precipitation CMIP6-NorESM2-SSP5-8.5 from 2015-01-01 to 2070-12-31
# note: once the transfer function is ready, we can apply it to the future scenario time series

# load data 
future <- "./Methods/pr_day_NorESM2-MM_ssp585_r1i1p1f1_gn_20150101-20701231_v20191108.nc"
...
mkt.bcssp <- mk.test(df.ssp$bcssp); mkt.bcssp #> p>0.05; NO hay tendencia
...
#write.csv(df.ssp,"./BiasCorrection/Routputs/rcode10_daily_NorESM2ssp585_bcm1219splited.csv",row.names = FALSE)
#save.image("./BiasCorrection/RWorkingSpace/rcode10_wspace.RData")
